...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderIssues(dataToRender, filter) {
        ...
        filtered.forEach(issue => {
            gridDiv.appendChild(createCard(issue));
        });
        issueList.appendChild(gridDiv);
    }
    ...
    // 5. Helper: Card Creation
    function createCard(issue) {
        // Priority logic for card
        const priorityClass = issue.priority.toLowerCase() === "high" ? "bg-red-100 text-red-800" : 
                            issue.priority.toLowerCase() === "medium" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800";
        ...
        card.innerHTML = `
            <div class="flex justify-between items-center">
                <img src="${statusIcon}" class="w-5 h-5" alt="${issue.status}">
                <p class="rounded-full px-3 text-[10px] font-bold ${priorityClass}">${issue.priority.toUpperCase()}</p>
            </div>
            <h3 class="font-bold text-sm">${issue.title}</h3>
            <p class="text-gray-600 text-sm line-clamp-2">${issue.description}</p>
            <div class="flex gap-2 flex-wrap">
                ${issue.labels.map(label => getLabelHTML(label)).join("")}
            </div>
            <div class="p-2 rounded shadow text-[10px] bg-white/50">
                <p class="font-semibold">#${issue.id} by ${issue.author}</p>
...
function getLabelHTML(label) {
    let colorClass = "border-violet-200 text-violet-600 bg-violet-50";

    
    if (label.toLowerCase() === "bug") {
        colorClass = "border-red-200 text-red-600 bg-red-50";
    } else if (label.toLowerCase() === "help wanted") {
        colorClass = "border-orange-200 text-orange-600 bg-orange-50";
    }

    
    return `<span class="border rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${colorClass}">
                ${label}
            </span>`;
}

    // 6. Helper: Modal Logic
    function openModal(id) {
        fetch(`https://phi-lab-server.vercel.app/api/v1/lab/issue/${id}`)
            .then(res => res.json())
            .then(response => {
                ...
                document.getElementById("modal-meta").innerText = `Opened by ${data.author} • ${dateStr}`;

                const priorityEl = document.getElementById("modal-priority");
                priorityEl.innerText = data.priority.toUpperCase();
                priorityEl.className = `px-4 py-1 rounded-full text-xs font-bold text-white ${data.priority.toLowerCase() === 'high' ? 'bg-red-500' : 'bg-blue-500'}`;
                ...
                statusEl.className = `px-3 py-1 rounded-full text-xs font-bold text-white ${data.status.toLowerCase() === 'open' ? 'bg-green-500' : 'bg-gray-500'}`;
